		...
		if (!$json) {
			if (isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'existing') {
				...
				if (!in_array($this->request->post['address_id'], array_keys($this->model_account_address->getAddresses()))) {
					foreach ($this->model_account_address->getAddresses() as $value) {
						if (isset($value['address_id'])) {
							$this->request->post['address_id'] = $value['address_id'];
						}
					}
				}
				// fix
				...
				if (empty($this->request->post['address_id'])) {
					$json['error']['warning'] = $this->language->get('error_address') . ' (1)';
				} elseif (!in_array($this->request->post['address_id'], array_keys($this->model_account_address->getAddresses()))) {
					// проблема была здесь 
					$json['error']['warning'] = $this->language->get('error_address') . ' (2)';
				}
				...
				if (!$json) {
					...
					$this->session->data['payment_address'] = '0';
