...
class ModelCheckoutOrder extends Model {
	...
	public function getProductCode($product_id, $option) {

			$query = $this->db->query("SELECT code FROM " . DB_PREFIX . "product_option_value WHERE product_option_id = '" . $product_id . "' AND option_value_id = '" . $option . "'");
			foreach ($query->rows as $codes) {
				$code = $codes['code'];
			}
		return $code;
	}
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				foreach ($product['option'] as $option) {
					...
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "', `code` = '". $this->getProductCode($option['product_option_id'], $option['option_value_id']) ."'");

					// foreach ($option['code'] as $options) {

					// 	$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET `code` = '". $this->db->escape($options['code']) ."' WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$order_product_id . "'");
	...
	public function updateOrderData($order_id, $field, $value) {
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET `$field` = '$value' WHERE `order_id` = '$order_id'");
	}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['text_code'] = $language->get('text_code');
				$data['text_quantity'] = $language->get('text_new_quantity');
				$data['text_price'] = $language->get('text_new_price');
				$data['text_total'] = $language->get('text_new_total');
				$data['text_footer'] = $language->get('text_new_footer');
				$data['option_language'] = $language->get('option_language');
				...
				$data['payment_method'] = $this->config->get('payment_method');
				$data['payment_method1'] = $order_info['payment_method'];
				$data['shipping_method'] = $order_info['shipping_method']; //Укрпошта
				$data['firstname'] = $order_info['firstname'];
				$data['text_lastname'] = ($order_info['language_id'] == 2)?"Фамилия":"Прiзвище";
				$data['lastname'] = $order_info['lastname'];

				$data['text_comment1'] = ($order_info['language_id'] == 2)?"Комментарий":"Комментар";
				$data['text_city1'] = ($order_info['language_id'] == 2)?"Город":"Мiсто";
				$data['text_map1'] = ($order_info['language_id'] == 2)?"Карта проезда к нам":"Карта проїзду до нас";
				$data['city1'] = $order_info['payment_city']?$order_info['payment_city']:$order_info['shipping_city'];
				$data['comment1'] = $order_info['comment'];

				$data['text_company'] = ($order_info['language_id'] == 2)?"Отделение новой почты":"Номер вiддiлення";


				$data['text_postcode'] = ($order_info['language_id'] == 2)?"Индекс":"Iндекс";
				if($order_info['shipping_method'] == "Укрпошта" or $order_info['shipping_method'] == "Укрпочта"){
					$data['company'] = false;
					$data['postcode'] = $order_info['payment_postcode'];
				}else{
					$data['postcode'] = false;
					$data['company'] = $order_info['payment_company']?$order_info['payment_company']:$order_info['shipping_company'];
				}
			...
			//	$mail->setFrom($this->config->get('config_email'));
				...
				if ($this->config->get('config_order_mail')) {
					...
					foreach ($order_product_query->rows as $product) {
						...
						foreach ($order_option_query->rows as $option) {
							$text .= $product['quantity'] . 'x ' . $product['name'] . ' (' . $option['code'] . ') ' . html_entity_decode($this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8') . "\n";
