    ...
    <section class="cart_inner centered">
        ...
        <?php } ?>
            ...
            <div id="simplecheckout_form_<?php echo $group ?>" <?php echo $display_error && $has_error ? 'data-error="true"' : '' ?> <?php echo $logged ? 'data-logged="true"' : '' ?>>
                <div class="simplecheckout ch_tabs">
                    <?php if (!$cart_empty) { ?>
                        <?php if ($steps_count > 1) { ?>
                            ...
                            <?php } else { ?>
                                <div id="simplecheckout_step_menu" class="checkout_steps">
                                    <?php for ($i=1;$i<=$steps_count;$i++) { ?>
                                    <div class="simple-step tab_name checkout_step checkout_tbs" data-onclick="gotoStep" data-step="<?php echo $i; ?>">
                                        <img src="image/checkout/checkout_<?php echo $i; ?>.svg" alt="">
                                        <div class="ch_step_name"><?php echo $step_names[$i-1] ?></div>
                                    </div><?php if ($i < $steps_count) { ?><span class="simple-step-delimiter" data-step="<?php echo $i+1; ?>"></span><?php } ?><?php } ?>
                                </div>
                            <?php } ?>
                        <?php } ?>
                        ...
                        <div class="simplecheckout-button-block buttons" id="buttons" style="text-align:left!important;">
                            <div class="simplecheckout-button-right" style="text-align:left!important;float:left;">
                                <?php if ($display_agreement_checkbox) { ?>
                                    <span id="agreement_checkbox">
                                        <?php foreach ($text_agreements as $agreement_id => $text_agreement) { ?>
                                            <div class="checkbox"><input type="checkbox" name="agreements[]" value="<?php echo $agreement_id ?>" <?php echo in_array($agreement_id, $agreements) ? 'checked="checked"' : '' ?> /> <label></label> <span><?php echo $text_agreement; ?></span></div>
                                        <?php } ?>
                                    </span>
                                <?php } ?>
                                <?php if ($steps_count > 1) { ?>
                                    <a class="button btn-primary button_oc btn_grad btn" data-onclick="nextStep" id="simplecheckout_button_next"><span><?php echo $button_next; ?></span></a>
                                <?php } ?>
                                <a class="button btn-primary button_oc btn_grad btn" <?php echo $block_order ? 'disabled' : '' ?> data-onclick="createOrder" id="simplecheckout_button_confirm"><span><?php echo $button_order; ?></span></a>
                            </div>
                            <div class="simplecheckout-button-left">
                                <?php if ($display_back_button) { ?>
                                    <a class="button btn-primary btn_grad button_oc btn" data-onclick="backHistory" id="simplecheckout_button_back"><span><?php echo $button_back; ?></span></a>
                                <?php } ?>
                                <?php if ($steps_count > 1) { ?>
                                    <a class="button btn-primary btn_grad button_oc btn" data-onclick="previousStep" id="simplecheckout_button_prev"><span><?php echo $button_prev; ?></span></a>
                                <?php } ?>
                            </div>
                        </div>
