<div class="form-group <?php if ($required) { ?>required<?php } ?> row-<?php echo $id ?>">
    <label class="control-label <?php echo $page == 'checkout' ? 'col-sm-4' : 'col-sm-2' ?>" for="<?php echo $id ?>"><?php echo $label ?><?php if ($required) { ?>*<?php } ?></label>
      ...
      <?php } elseif ($type == 'radio') { ?>
        ...
        <?php foreach ($values as $info_id => $info) { ?>
          <div class="radio">
            ...
            <label for="<?php echo $id ?>_<?php echo $info_id ?>"></label><span><?php echo $info['text'] ?></span>
          </div>
        <?php } ?>
        </div>
      <?php } elseif ($type == 'checkbox') { ?>
        ...
        <?php foreach ($values as $info_id => $info) { ?>
          <div class="checkbox">
            ...
            <input type="checkbox" name="<?php echo $name ?>[<?php echo $info['id'] ?>]" id="<?php echo $id ?>_<?php echo $info_id ?>" value="1" <?php echo !empty($value[$info['id']]) ? 'checked="checked"' : '' ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>><label></label><span><?php echo $info['text'] ?></span>
          </div>
        <?php } ?>
        </div>
